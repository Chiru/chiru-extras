        ...
        if len(self.path) > 0 and self.path != None:
            if self.destnumber == len(self.path):
                ...
                if RANDOM_MODE == False:
                    if self.destweights[self.hotspot]==[]: #Comment these out to if going with random targets
                        hs,gl = pick_first(self.origins) #Comment these out to if going with random targets
                    else: hs, gl = pick_second(self.hotspot, self.destweights) #Comment these out to if going with random targets
                    #print 'going random'
                    if hs == 2001 or hs == 2002 or hs == 2003 or hs == 2004: #Comment out if you want to try random mode
                        self.exiting = True #Comment out if you want to try random mode
                        self.get_closest_exit(hs) #Comment out if you want to try random mode
                        return #Comment out if you want to try random mode
                        #print 'Im exiting'
                    self.hotspot = hs #Comment out if you want to try random mode
                    self.set_goal(gl) #Comment out if you want to try random mode
                else: self.set_random_goal() #only for testing
                self.set_path()
                if len(self.path) == 0:
                    return
            self.speed = 2
